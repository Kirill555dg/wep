...
Серверная часть автоматически формирует OpenAPI-спецификацию, которая используется как машинно-читаемое описание контракта API. Это важно не только для ручной проверки эндпоинтов, но и для интеграции с клиентской частью, поскольку OpenAPI-описание может выступать основой для кодогенерации, тестирования и валидации совместимости версий интерфейса. На рисунке @openapi-ui приведено место для скриншота интерфейса OpenAPI-документации.

#figure(
  rect(width: 100%, height: 6cm, fill: luma(230))[
    #align(center + horizon)[_Скриншот: Swagger UI / OpenAPI-документация_]
  ],
...
Автоматизированное тестирование выполняется с помощью `pytest` и служит средством проверки корректности реализованных сценариев. Тесты охватывают регистрацию и аутентификацию, автоматическую проверку ответов, подсистему обмена сообщениями, OpenAPI-контракт, расчёт прогресса и работу realtime-компонентов. На рисунке @test-results приведено место для скриншота результата выполнения набора тестов серверной части.

#figure(
  rect(width: 100%, height: 5.5cm, fill: luma(230))[
    #align(center + horizon)[_Скриншот: успешный прогон pytest (30 passed)_]
  ],
  caption: [Результат выполнения набора автоматизированных тестов серверной части],
) <test-results>
...
Интерфейс страницы преподавателя со списком классов должен решать две практические задачи: быстро показать состав учебных групп и дать доступ к операциям управления ими. На рисунке @ui-classrooms предполагается отображение карточек классов или таблицы с краткой информацией о предмете, количестве студентов и коде приглашения, что позволяет преподавателю быстро ориентироваться в текущем состоянии учебного процесса.
...
Страница урока для студента должна концентрировать в одном месте теоретический материал, условия домашнего задания и навигацию по связанным сущностям курса. На рисунке @ui-lesson предполагается интерфейс, в котором студент может последовательно изучить материал, перейти к заданиям и видеть контекст конкретного урока, не покидая страницу.
...
Интерфейс отображения результатов тестирования важен как элемент обратной связи: он должен не просто показывать итоговый балл, но и позволять пользователю понять, как система оценила его попытку. На рисунке @ui-testing предполагается отображение количества попыток, набранных баллов, статуса проверки и, при необходимости, детализации по отдельным задачам.
